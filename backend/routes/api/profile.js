...
//@desc  Createe/add or update users profile
//@access Private
router.post(
  '/',
  [
...
// @desc     Get profile by user ID [so that every user can get his profile]
// @access   Public
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      //Fetch the profile we want  to add the experience to
      //req.user.id we get it from the  token
      const profile = await Profile.findOne({ user: req.user.id });
      //unshift is the same as push
      console.log(profile);
